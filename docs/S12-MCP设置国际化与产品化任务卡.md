...
截至 2026-08-19，当前工作区已通过禁用构建缓存的独占本地统一候选门：744/744 平台测试，15389/18234（84.40%）整体行覆盖率，各分区覆盖率、SBOM、本地化、结构和最低 `IU-252.28539.54` Verifier 均通过；临时密钥签名、验签、篡改拒绝、归档身份、加固生命周期 1/1、可选依赖隔离 5/5 以及发布静态契约均通过。这些是本地非生产证据，不替代新 HEAD 远端、生命周期 100/100、Environment 审批、生产密钥签名、Marketplace 接收或副屏验收。
