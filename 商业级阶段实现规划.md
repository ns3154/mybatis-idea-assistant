# MyBatis IDEA Assistant 商业级阶段实现规划

> 文档版本：0.1
> 编写日期：2026-08-09
> 当前状态：规划基线，尚未创建插件源码
> 初始目标平台：IntelliJ IDEA Ultimate 2026.1（Build 261）
> 最终目标：达到可长期维护、可公开发布的商业级 MyBatis IDE 插件质量

## 1. 文档目的

本文用于指导 `mybatis-idea-assistant` 从零实现为完整商业级 IntelliJ IDEA 插件。它不是单纯的功能愿望清单，而是后续开发、测试、验收和发布的共同基线。
...
- 具备商业发布所需的兼容验证、签名、文档、隐私、安全和回滚能力。

### 2.2 “商业级完成”的定义

商业级完成不以“菜单已经出现”判断，而以以下结果共同判断：
...
| 维度 | 第一阶段目标 | 商业版目标 |
|---|---|---|
| IDE | IntelliJ IDEA Ultimate 2026.1 / 261 | 最近四个稳定大版本，按兼容适配层维护 |
...
| IDE 版本 | Ultimate | Ultimate、Community、Android Studio |
...
| 方法名生成 | JPA 风格查询、更新、删除、统计、排序、分页、批量、动态条件 | P1 |
...
| S13 | 性能、可靠性、跨版本回归和商业发布 | 6～10 周 | RC、无阻断缺陷、商业级 1.0 |
...
- 目标固定为 IDEA Ultimate 2026.1 / 261。
...
这些数值是初始商业级预算。调整必须有基准数据并记录架构决策，不能为了让测试变绿而随意放宽。
...
| Release Candidate | S12～S13 | 商业候选 | MCP、产品化、全矩阵兼容、性能和安全收口 |
| 1.0 | 全阶段通过 | 商业发布 | 无 P0/P1、完整文档、签名发布和回滚能力 |
...
后续开始编码时，第一批只做 S1 和 S2 的最小纵切面：

1. 创建现代插件工程并构建可安装 ZIP。
2. 建立一个 Java + MyBatis 样例项目。
3. 识别 Mapper 接口和对应 XML namespace。
4. 建立 statement id 索引。
5. 实现单个 Mapper 方法到 XML statement 的跳转。
6. 为这条链路补齐平台测试、沙箱实测和 Plugin Verifier。

第一批明确不实现数据库生成、动态 SQL、OGNL、MCP 和商业发布功能。只有最小纵切面满足 S1/S2 的线程、索引和测试规范后，才进入 S3。
