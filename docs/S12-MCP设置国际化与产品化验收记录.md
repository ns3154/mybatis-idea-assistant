...
- 产品化：中英文资源键对称；设置、MCP、数据库界面、生成器、方法名 SQL 及完整 SQL 工具链完成资源化，并由 `verifyLocalizedUserInterface` 阻止新增硬编码中文字符串；明暗 SVG 图标、Apache-2.0 LICENSE/NOTICE、隐私说明、第三方清单、发布升级/回滚说明已加入。
...
- 整体行覆盖率：13784/16512，83.48%；S12 产品化核心：1065/1204，88.46%；均高于 70%/85% 阻断线。
...
- SQL 工具链、生成器、方法名 SQL/Wrapper/Join、统一语义模型及重构安全诊断的解析、校验、冲突、回滚与候选注释已完成资源化并纳入静态门；动态 SQL、OGNL 等历史核心诊断仍可能透传固定中文，仍需继续迁移，不得提前宣称全产品英文错误文案完成。
