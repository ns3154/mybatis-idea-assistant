...
- S7～S10 分组测试 329/329 通过，覆盖方法 AST、六方言 SQL、set-based `insertBatch`、IN/NOT IN 失败关闭、Wrapper 版本门、显式 FK↔PK Join 和取消边界；全量 Gradle 门 760/760、84.40% 行覆盖率通过。
- Plus 3.5.17/Flex 1.11.8 Wrapper 输出仍通过语义语料的真实编译契约；不外推未验证版本，也不把列名当 FK 关系。
- 副屏三个 Database Tools 动作与真实框架安装交互仍待外部环境。
