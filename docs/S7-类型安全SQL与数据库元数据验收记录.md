...
- S7～S10 分组测试 329/329 通过，包含有界代表 SQL、SQL PSI/source map、Database Tools 只读适配、JDBC 显式配置、取消/过期快照和保守诊断；全量 Gradle 门 760/760、84.40% 行覆盖率通过。
- Database Tools 动作描述已从描述符硬编码中文改为中英文资源键，并通过目标动作、Bundle、本地化和插件结构门。
- Database Tools 实机元数据变化、真实驱动/数据库、PasswordSafe 和副屏 Undo 仍待外部环境；未把文本方言夹具宣称为真实 DM8 验收。
